<template>
  <v-card>
    <v-card-title class="display-1">Verify E-mail.</v-card-title>
    <v-divider />
    ...
    <v-card-text v-show="showError == true" class="text-xs-center">
      Error occurs while verifying e-mail.
...
export default {
  ...
  async asyncData({ query, store }) {
    const isSuccess = await store.dispatch('user/verifyEmail', query['code'])
